...
public:
    ...
    virtual void visitData(const SpatialIndex::IData& in, SpatialIndex::id_type id) {
        ...
        if (!payload->isCreated) {
			...
			const int ps = payload->pageSize;
            int8* page = new int8[ps];
            int8* ptr = page;
...
FLATTest::FLATTest(char* filename, int len) {
	//std::vector<char> *fname2 = new std::vector<char>[len];
	std::string filename2(filename, len);
	inputStem = filename2 + "\\FLAT168";
    //char * newInputStem = strcat(filename, "FLAT168_index.dat");
	//inputStem = strcat_s(filename, len, "\\FLAT168_index.dat");
    cout << inputStem << endl;
