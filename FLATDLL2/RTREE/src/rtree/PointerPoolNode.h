...
namespace Tools
{
	template<> class PointerPool<SpatialIndex::RTree::Node>
	{
	public:
		...
		~PointerPool()
		{
			...
			while (! m_pool.empty())
			{
				SpatialIndex::RTree::Node* x = m_pool.top(); m_pool.pop();
				#ifndef NDEBUG
				--m_pointerCount;
				#endif
				delete x;
		...
		PoolPointer<SpatialIndex::RTree::Node> acquire()
		{
			if (! m_pool.empty())
			{
				SpatialIndex::RTree::Node* p = m_pool.top(); m_pool.pop();
				#ifndef NDEBUG
				++m_hits;
				#endif

				return PoolPointer<SpatialIndex::RTree::Node>(p, this);
			}
			#ifndef NDEBUG
			else
			{
			...
			return PoolPointer<SpatialIndex::RTree::Node>();
		}

		void release(SpatialIndex::RTree::Node* p)
		{
			if (p != 0)
			{
	...
	protected:
		uint32_t m_capacity;
		std::stack<SpatialIndex::RTree::Node*> m_pool;
